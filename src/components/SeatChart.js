...
const SeatChart = ({ occasion, bookAia, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await bookAia.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await bookAia.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
